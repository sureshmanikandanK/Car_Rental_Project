...
const BlogDetails = () => {
  ...
  useEffect(() => {
    fetch(`http://127.0.0.1:8000/blogs/blogs//${slug}/`) // Update with your Django API endpoint
      ...
      <section>
        <Container>
          <Row>
            <Col lg="8" md="8">
              <div className="blog__details">
                <img src={`http://127.0.0.1:8000/blogs/blogs/${blog.imgUrl}`} alt="" className="w-100" />
                <h2 className="section__title mt-4">{blog.title}</h2>
            ...
            <Col lg="4" md="4">
              ...
              {blog.recent_posts?.map((item) => (
                <div className="recent__blog-post mb-4" key={item.id}>
                  <div className="recent__blog-item d-flex gap-3">
                    <img src={`http://localhost:8000${item.imgUrl}`} alt={item.title} className="w-25 rounded-2" />
                    <h6>
